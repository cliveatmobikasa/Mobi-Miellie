...
var collectionHandle = $('#collection-handle').val();
var collectionId = $('#collection-id').val();
var collectionManual = $('#collection-manual-sorting').val();

var base_url = 'https://www.searchanise.com/getresults?api_key=2F1Y7q8F2Q&output=jsonp&items=true&facets=true&facetsShowUnavailableOptions=false&categories=true&collection='+collectionHandle;
var startIndex = 0;
var move_to_top = false;
var escPressed = false;

function SearchResult() {
    const container = document.querySelector('.collection-result-main');
    if(!container) return;
    var sort_by = document.querySelector('[data-sortby] select').value;
      if(collectionManual === 'true'){
        sort_by ='collection_'+collectionId+'_position&sortOrder=asc';
        $('body').addClass('ascCollection');
      }
    // var sort_by ='collection_'+collectionId+'_position&sortOrder=asc'; //document.querySelector('[data-sortby] select').value;
    var initial_max_result = 20;
    
        var initialURL = base_url + '&sortBy=' + sort_by + '&startIndex='+startIndex+'&maxResults=' + initial_max_result;
    
        var filter_items = document.querySelector('.collection-result-main').querySelectorAll('[data-filter-items] [data-filter-item]');
    if(window.innerWidth < 799) {
      var filter_items = document.querySelector('.collection-result-main').querySelectorAll('[data-filter-mobile-items] li');
    }
    ...
    $('.collection-result-status').attr('aria-hidden', 'false');
    $('.collection-result-status').text('Loading');

    setTimeout(() => {
      $('.collection-result-status').attr('aria-hidden', 'true');
    }, 1000);
...
function fetchSearchResults(fetch_url) {
    ...
    fetch(fetch_url).then(function(res){
      ...
      $('.skeleton-main-container').attr('style',' ');
        ...
        $('.collection-result-status').attr('aria-hidden', 'false');
        $('.collection-result-status').text(data.totalItems + ' products found');
        ...
        if(data.totalItems && data.totalItems > 0) {
            $('.collection-result-main').removeClass('no--result hidden');
            ...
            $('.collection-result-main').addClass('no--result hidden');
...
if(available_prod_qty == true ){
    ...
    if(move_to_top === true) {
        $('html, body').animate({
            scrollTop: $(".collection-result-main").offset().top - 150
        }, 2000);
    }
    ...
    if(total_page > 1 ) {
        var pagination_html = '<div class="paginate"><nav role="navigation" aria-label="pagination"><ol class="pagination">';
            ...
            for (i = 0; i < total_page; i++) {
                ...
                if(current_page == i) {
                    pagination_html += '<li class="page current active" class="visually-hidden" data-page="'+(i+1)+'" aria-current="page"><span class="visually-hidden">Page </span> '+(i+1)+'</li>';
                } else {
...
function display_search_categories(data) {
  ...
  if(data.totalItems === 0) {
    $('[data-searchpage-collections]').html('<p>No collection found.</p>');
    return;
  }
  ...
  if(!data_categories || data_categories.length === 0) {
    ...
    let data_categories_html = '<ul role="list">';
    data_categories.forEach(function(data_category){
      data_categories_html += '<li role="listitem"><a class="outline-inset" href="'+data_category.link+'">' + data_category.title + '</a></li>';
    });
    data_categories_html += '</ul>';
    $('[data-searchpage-collections]').html(data_categories_html);
  }
}
...
function display_search_blogs(data) {
  ...
  if(data.totalItems === 0) {
    $('[data-searchpage-blogs]').html('<p>No blog found.</p>');
    return;
  }
  ...
  if(!data_pages || data_pages.length === 0) {
    ...
    let data_pages_html = '<ul role="list">';
    data_pages.forEach(function(data_page){
      data_pages_html += '<li role="listitem"><a class="outline-inset" href="'+data_page.link+'">' + data_page.title + '</a></li>';
    });
    data_pages_html += '</ul>';
    $('[data-searchpage-blogs]').html(data_pages_html);
  }
}
...
function numberGenerator(){
    $('.collection-result-main .plp-filter-items li .btn-filter-title').each(function(e){
...
var MobileFilterArr = [];
  ...
  $(document).on("click",".btn.apply_btn", function(){
    $("html").animate({ scrollTop: 0 }, 1500);
  });
  $(document).on("click","#sort-by-mobile_search li", function(){
    $("html").animate({ scrollTop: 0 }, 1500);
  });
